...
// TestDrainReclaim_QueueDrivenReclaim covers the governor's steady-state path: a
// vertex that reaches refs==0 (a dropped adoption, a born-at-zero cache fetch) is
// freed by drainReclaim alone — no full-map scan — while a still-referenced vertex
// and a pinned system key survive. This is the path memoryGovernorLoop runs every
// tick under target; reclaimScan only backstops it under memory pressure.
func TestDrainReclaim_QueueDrivenReclaim(t *testing.T) {
	...
	// Adopted then dropped: incref over the creation ref, then two decrefs drive it
	// back to 0 — only the last transition enqueues it.
	dropTip := Tip{1, 4}
	...
	// A second drain with nothing queued must be a no-op (and must not block on the
	// empty UMPSCQueue).
	if freed := p.drainReclaim(); freed != 0 {
